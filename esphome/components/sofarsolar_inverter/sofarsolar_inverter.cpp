    ...
    namespace sofarsolar_inverter {
        ...
        void SofarSolar_Inverter::loop() {
			if (millis() - zero_export_last_update > 1000 && this->zero_export_) {
				...
				int percentage = (G3_dynamic.at(TOTAL_ACTIVE_POWER_INVERTER).sensor->state + this->power_sensor_->state) * 10000 / model_parameters.at(this->model_id_).max_output_power_w;
				if (percentage < 0) {
					percentage = 0;
