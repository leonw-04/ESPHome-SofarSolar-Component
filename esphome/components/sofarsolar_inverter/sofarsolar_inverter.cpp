...
namespace esphome {
    namespace sofarsolar_inverter {
        ...
        static const uint16_t SofarSolar_Register[3][] = [ // 0 uint16_t, 1 int16_t, 2 uint32_t, 3 int32_t, 4 float16, 5 float32
            [0x0684, 2, 2], // PV Generation Today
            [0x0686, 2, 2], // PV Generation Total
            [0x0688, 2, 2], // Load Consumption Today
            [0x068A, 2, 2], // Load Consumption Total
            [0x0694, 2, 2], // Battery Charge Today
            [0x0696, 2, 2], // Battery Charge Total
            [0x0698, 2, 2], // Battery Discharge Today
            [0x069A, 2, 2], // Battery Discharge Total
            [0x0485, 1, 1], // Total Active Power Inverter
            [0x0584, 1, 0], // PV Voltage 1
            [0x0585, 1, 0], // PV Current 1
            [0x0586, 1, 0], // PV Power 1
            [0x0588, 1, 0], // PV Voltage 2
            [0x0589, 1, 0], // PV Current 2
            [0x058A, 1, 0], // PV Power 2
            [0x05C4, 1, 0], // PV Power Total
            [0x0667, 1, 1], // Battery Power Total
            [0x0668, 1, 0], // Battery State of Charge Total
            [0x1187, 2, 3], // Desired Grid Power
            [0x1189, 2, 3], // Minimum Battery Power
            [0x118B, 2, 3], // Maximum Battery Power
            [0x1110, 1, 0], // Energy Storage Mode
            [0x1044, 1, 0], // Battery Conf ID
            [0x1045, 1, 0], // Battery Conf Address
            [0x1046, 1, 0], // Battery Conf Protocol
            [0x1050, 1, 0], // Battery Conf Voltage Nominal
            [0x1047, 1, 0], // Battery Conf Voltage Over
            [0x1048, 1, 0], // Battery Conf Voltage Charge
            [0x1049, 1, 0], // Battery Conf Voltage Lack
            [0x104A, 1, 0], // Battery Conf Voltage Discharge Stop
            [0x104B, 1, 0], // Battery Conf Current Charge Limit
            [0x104C, 1, 0], // Battery Conf Current Discharge Limit
            [0x104D, 1, 0], // Battery Conf Depth of Discharge
            [0x104E, 1, 0], // Battery Conf End of Discharge
            [0x104F, 1, 0], // Battery Conf Capacity
            [0x1051, 1, 0], // Battery Conf Cell Type
            [0x1052, 1, 0], // Battery Conf EPS Buffer
            [0x1053, 1, 0] // Battery Conf Control
        ]
        ...
        void SofarSolar_Inverter::update() {
            std::vector<uint8_t> frame;
            for (int i = 0; i < sizeof(SofarSolar_Register) / sizeof(SofarSolar_Register[0]); i++) {
                frame.clear();
                uint16_t start_address = SofarSolar_Register[i][0];
                uint16_t quantity = SofarSolar_Register[i][1];
                frame.push_back(this->modbus_address_);
                frame.push_back(0x03); // Read multiple registers
                frame.push_back(start_address >> 8); // High byte of start address
                frame.push_back(start_address & 0xFF); // Low byte of start address
                frame.push_back(quantity >> 8); // High byte of quantity
                frame.push_back(quantity & 0xFF); // Low byte of quantity
                send_read_modbus_registers(frame);
                this->set_timeout(150); // Set a timeout for the response
        ...
        void SofarSolar_Inverter::receive_modbus_response(std::vector<uint8_t> &response) {
            ...
            if (uart->available()) {
                ...
                ESP_LOGD(TAG, "Received Modbus response: %s", frame_to_string(response).c_str());
                if (!check_crc(response)) {
                    ESP_LOGE(TAG, "CRC check failed for received frame");
                }
            } else {
                ESP_LOGW(TAG, "No data available on UART");
            }
        }
