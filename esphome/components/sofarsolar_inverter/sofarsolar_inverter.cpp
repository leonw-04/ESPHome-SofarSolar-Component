    ...
    namespace sofarsolar_inverter {
        ...
        void SofarSolar_Inverter::loop() {
            ...
            for (auto &pair : registers_G3) {
                ...
                ESP_LOGW(TAG, "Checking register %04X: Time since last update: %d ms, Update interval: %d ms", pair.second.start_address, (millis() - pair.second.timer), pair.second.update_interval * 1000);
