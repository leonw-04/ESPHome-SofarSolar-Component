    ...
    namespace sofarsolar_inverter {
        ...
        void SofarSolar_Inverter::loop() {
            ...
            for (auto &pair : registers_G3) {
                ...
                ESP_LOGW(TAG, "Checking register %04X: Time since last update: %d seconds, Update interval: %d seconds", pair.second.start_address, millis() / 1000 - pair.second.timer, pair.second.update_interval);
        ...
        void SofarSolar_Inverter::dump_config(){
            ...
            for (const auto &reg : registers_G3) {
                ESP_LOGCONFIG(TAG, "  %s: start_address = %04X, type = %d, scale = %f, default_value = %s, enforce_default_value = %s, write_set_value = %s",
                              ...
                              reg.second.is_default_value_set ? reg.second.default_value.to_string().c_str() : "Not set",
                              TRUEFALSE(reg.second.enforce_default_value), TRUEFALSE(reg.second.write_set_value));
            }
        }
